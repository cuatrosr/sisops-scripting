...
while true; do
	...
	read -r -p "Ingrese el nombre completo del usuario: " full_name
	...
	if echo "$useradd_output" | grep -q "useradd"; then
		...
		while true; do
			...
			read -r -p "Ingrese una contraseña para el usuario: " password
			echo
			read -r -p "Confirme la contraseña: " password_confirm
			echo
